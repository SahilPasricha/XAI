  ...
  def save_weights(self, filepath,cur_epoch, skip_epoch,skip_from, weight_pred_ind=0,overwrite=True, save_format=None):
    ...
    for x in range(np_weight_counter):
        if (x != 100):
            ...
            csv_file_current = csv_file_head + str(x) + ".csv"
            
            if os.path.exists(csv_file_current):
                df = pandas.read_csv(csv_file_current)
                header = str(df.shape[1])
                ...
                if(cur_epoch == skip_from):
                    if (weight_pred_ind !=0 ):                            
                        ...
                        temp_file = csv_file_head_orig + str(x) + ".csv"
    ...
    if os.path.exists(csv_file):
        ...
        print("Writing weights to CSV")
        np.savetxt(csv_file , np_acc_weight[1:] , fmt='%s', delimiter=',',header="Epoch:1", comments='')
