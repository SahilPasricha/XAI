...
class Network(base_layer.Layer):
    ...
    def save_weights(self, filepath,cur_epoch,skip_info=[], weight_pred_ind=0,overwrite=True, save_format=None):
        #cur_epoch = cur_epoch + 1 #as epoch start frp,0 but we want to present them from 1 in CSV and logics
        import sys
        from WeightPredictions.RegFunc import calc_reg
        import os
        if(len(skip_info)):
            skips = np.array(skip_info)[:,0]
        else:
            skips = [-1]#always false


        #Add an if statement to avoid saving weights for skipped epochs
        csv_path=os.path.dirname(os.path.dirname(filepath))
        csv_file_orig = csv_path + "/weights/Oweights.csv"
        csv_file_head_orig = csv_path + "/weights/Oweights"
        csv_file_reg = csv_path + "/weights/Rweights.csv"
        csv_file_head_reg = csv_path + "/weights/Rweights"

        #For model with skip epoch
        if (weight_pred_ind !=  0):
            csv_file = csv_file_reg
            csv_file_head = csv_file_head_reg

        #For model without skip epoch
        else:
            csv_file = csv_file_orig
            csv_file_head = csv_file_head_orig



        weight = self.get_weights()
        np_weights = np.array(weight)
        np_weight_counter = np_weights.shape[0]
        np_acc_weight =  np.array([0])
        #import pdb;pdb.set_trace();
        print("***************Writing csv weights for Epoch :%d ******* " %cur_epoch)


        for x in range(np_weight_counter):
            if (x != 100):

                #will be alwys true
                layer_weights = np_weights[x].flatten()
                layer_weights = layer_weights.round(decimals=4)
                np_acc_weight = np.concatenate((np_acc_weight,layer_weights))
                csv_file_current = csv_file_head + (str(x).zfill(2)) + ".csv"

                if  not os.path.exists(csv_file_current):
                    ids = np.arange(layer_weights.shape[0])
                    np.savetxt(csv_file_current, ids , fmt='%s', delimiter=',',header='Id',comments='')

                df = pandas.read_csv(csv_file_current)
                header = str(df.shape[1]-1)
                df [header] = layer_weights
                df.to_csv(csv_file_current, index = False)


                # problem --> it shall do in order --> copy new csv, write weights , write reg weights
                if(cur_epoch+1 in skips):
                    print("***************Starting Reg at Epoch :%d ******* " %cur_epoch)
                    if (weight_pred_ind !=0 ):
                        #import pdb ;pdb.set_trace()
                        if (cur_epoch+1 == skips[0]):
                            print("\n Duplicating Non Reg CSV for layer :%d" %x)
                            temp_file = csv_file_head_orig + (str(x).zfill(2)) + ".csv"
                            command = "cp %s  %s" %(csv_file_current,temp_file)
                            os.system(command)
                            #copy histrity logs
                            new_file =  csv_path + "/weights/" + "orig_model_history_log.csv"
                            old_file = csv_path + "/weights/"  + "reg_model_history_log.csv"
                            command = "cp %s  %s" %(old_file,new_file)
                            os.system(command)

                        print("Predicting Weights for layer :%d at epoch %d" %(x, cur_epoch))

                        calc_reg(csv_file_current,np.array(skip_info)[np.argwhere((np.array(skip_info))[:,0] == cur_epoch+1)][0][0][1])
                        df  = pandas.read_csv(csv_file_current)
                        last_rec = str(df.shape[1]-2) #reducing 1 for 0 and one for ID
                        layer_predicted_weight = np.array(df[last_rec])
                        layer_shape = (weight[x]).shape
                        layer_predicted_weight = layer_predicted_weight.reshape(layer_shape)
                        weight[x] = layer_predicted_weight



                    #np.savetxt(csv_file_current, layer_weights , fmt='%s', delimiter=',',header='',comments='')

        np_acc_weight = np_acc_weight.round(decimals=4)
        if not os.path.exists(csv_file):
            print("Creating  Main CSV")
            ids = np.arange(np_acc_weight.shape[0] -1 )
            np.savetxt(csv_file , ids , fmt='%s', delimiter=',',header="Id", comments='')

        df = pandas.read_csv(csv_file)
        header = "Epoch:" + str(df.shape[1]-1)
        np_acc_weight =  np_acc_weight[1:]
        df [header] = np_acc_weight
        ...
        if(cur_epoch+1 in skips):
            if(weight_pred_ind!=0):
                if(cur_epoch+1 == skips[0]):
                    print("Duplicating model weights CSV ")
                    command = "cp %s  %s" %(csv_file,csv_file_orig)
                    os.system(command)
                self.set_weights(weight)
                print("\n Feeding Pridicted Weights to Model at Epoch :%04d " % (cur_epoch))


            #np.savetxt(csv_file , np_acc_weight[1:] , fmt='%s', delimiter=',',header="Epoch:0", comments='')
