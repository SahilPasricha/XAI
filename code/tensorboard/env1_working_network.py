...
class Network(base_layer.Layer):
  ...
  def save_weights(self, filepath,cur_epoch, skip_epoch,skip_from, overwrite=True, save_format=None):
    #Skipping epochs
    import sys
    sys.path.insert(0, '/home/pasricha/WeightPredictions') #Tailor
    from RegFunc import calc_reg
    import os
#     if(cur_epoch == skip_from):
#         print("\n skipping Epoch save from %d to %d",cur_epoch,skip_to)
#         import sys
#         sys.path.insert(0, '/home/pasricha/WeightPredictions') #Tailor
#         from RegFunc import calc_reg
#         calc_reg("/home/pasricha/keras_log-weights/log/cifar100/weights/weights0.csv",2)
        #Try predicting , input --> Csv, Output --> np array, lists , set weights
        #Set weights":
    ...
    csv_file = csv_path + "/weights/weights.csv"
    csv_file_head = csv_path + "/weights/weights"
    ...
    for x in range(np_weight_counter):
        if (x ==0 or x == 2  or x == 4):
            ...
            if os.path.exists(csv_file_current):
                ...
                if(cur_epoch == skip_from and skip_epoch !=0 ):
                ...
                np.savetxt(csv_file_current, layer_weights , fmt='%s', delimiter=',',header='0',comments='')

    np_acc_weight = np_acc_weight.round(decimals=4)
    if os.path.exists(csv_file):
        print("already exists")
        df = pandas.read_csv(csv_file)
        header = "Epoch:" + str(df.shape[1])
        ...
        if(cur_epoch == skip_from and skip_epoch!=0):
            ...
            print("Pridicted Weights Fed to Model")
        #else:
            #df.to_csv(csv_file,index= False)
