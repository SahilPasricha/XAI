          ...
          if current is None:
            ...
            if self.monitor_op(current, self.best):
              ...
              if self.save_weights_only:
                self.model.save_weights(filepath, weight_pred_ind = self.weight_pred_ind, cur_epoch=epoch, overwrite=True,skip_epoch= self.skip_epoch,skip_from=self.skip_from)
              else:
                self.model.save(filepath, weight_pred_ind = self.weight_pred_ind, cur_epoch=epoch, overwrite=True,skip_epoch= self.skip_epoch,skip_from=self.skip_from)
                self.model.save_weights(filepath, weight_pred_ind = self.weight_pred_ind, cur_epoch=epoch, overwrite=True,skip_epoch= self.skip_epoch,skip_from=self.skip_from)
            else:
              if self.verbose > 0:
